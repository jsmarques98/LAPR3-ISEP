...
public class MapGraph<V, E> extends CommonGraph<V, E> {
    ...
    public int getKey(V vert) {
        return mapVertices.get(vert).getKey();
    }

    public V[] allkeyVerts() {
        V vertElem = null;
        for (MapVertex<V, E> vert : mapVertices.values()) {
    ...
    public Collection<Edge<V, E>> incomingEdges(V vert) {
        ...
        for (MapVertex<V, E> mv : mapVertices.values()) {
            collect.addAll(mv.getAllInEdges(vert));
        }
    ...
    public boolean addVertex(V vert) {
        ...
        MapVertex<V, E> mv = new MapVertex<>(numVerts, vert);
        vertices.add(vert);
